...
class LEFocusedDataGenerator(Sequence):
    def __init__(self, image_paths, mask_paths, batch_size, image_size, num_classes,
                 le_oversample_rate=3.0, shuffle=True, augment=True, stain_norm=True):
        ...
        print("Analyzing LE content in dataset...")
        for idx, mask_path in enumerate(tqdm(mask_paths)):
            try:
                mask = load_img(mask_path, color_mode="grayscale")
                mask_array = img_to_array(mask)
                if 2 in np.unique(mask_array):
                    self.le_images.append(idx)
                else:
                    self.non_le_images.append(idx)
            except:
                self.non_le_images.append(idx)
        
        print(f"  LE images: {len(self.le_images)}")
        print(f"  Non-LE images: {len(self.non_le_images)}")
    ...
    def create_sampling_indices(self):
        ...
        print(f"Total samples per epoch: {len(self.indexes)} (LE oversampled {self.le_oversample_rate}x)")
...
def main():
    print("="*80)
    print("PROGRESSIVE UNFREEZING WITH IMPROVED ARCHITECTURE")
    print("="*80)
